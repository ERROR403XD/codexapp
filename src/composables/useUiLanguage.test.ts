...
it('updates existing UI labels and parameterized messages when language changes', () => {
  ...
  vi.stubGlobal('document', { documentElement: { lang: '', dataset: {} }, dispatchEvent })
  ...
  expect(dispatchEvent).toHaveBeenCalledWith(expect.objectContaining({ type: 'codexapp:language-change' }))
  ...
  expect(t('raw external error /workspace')).toBe('raw external error /workspace')
  setUiLanguage('en')
  expect(label.value).toBe('Current branch')
})
