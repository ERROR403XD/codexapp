...
9. Test from Mac via Tailscale: `curl --http1.1 http://<your-tailscale-host>:<port>/codex-api/thread-live-state?threadId=<id>` (use `--http1.1` to avoid Vite HTTP/2 upgrade hang).
