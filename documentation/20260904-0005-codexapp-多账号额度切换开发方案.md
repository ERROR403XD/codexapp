...
- [ ] 当前实例继续监听 `0.0.0.0:5900`；测试实例严格监听 `0.0.0.0:59001`，没有端口自动递增。
- [ ] 测试实例无密码且启动后不打开网页；CodexApp 启动未发起、代理或提示执行 Codex 登录。
- [ ] 测试实例能访问与当前实例相同的项目目录和 `cwd`，但拥有独立 `CODEX_HOME`、账号池和 app-server 进程。
- [ ] 源码、CLI help、依赖、构建产物、README 和有效测试中不再存在 CodexApp 内置 cloudflared/tunnel 功能。
...
| 新实例端口 | `lsof -nP -iTCP:59001 -sTCP:LISTEN` | 当前空闲；尚未部署 | 2026-09-04 |
...
- 当前状态：研究和修订版开发方案已完成，未修改功能代码、未部署 `59001`、未重新登录、未切换账号。
- 已验证内容：官方登录会先吊销同一 `CODEX_HOME` 当前认证；当前生产实例监听 `5900`；`59001` 空闲；已完成 cloudflared/tunnel 和启动登录代码面盘点。
- 未验证内容：新测试实例、tunnel 删除、启动无登录、账号去重 re-auth、新版数据模型、真实 token rotation、事务切换、线程连续性和性能指标均尚未实施。
- 下一步：用户审阅并确认修订方案后，从 P0 建立 `59001` 隔离实例开始，在 feature branch 中分阶段开发和测试。
- 关键路径：`src/cli/index.ts`、`src/server/accountRoutes.ts`、`src/server/codexAppServerBridge.ts`、`src/App.vue`、测试实例独立 `CODEX_HOME`。
- 操作限制：不得重启或覆盖当前 `codexapp.service`；不得让测试实例挂载生产 `/root/.codex`；不得在主 home 执行“添加账号”登录；不得改动工作区会话结构；不得记录或输出任何真实凭据；未获确认前不实施功能代码。
